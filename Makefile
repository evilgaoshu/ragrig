...
doctor:
	$(UV) run python -m scripts.doctor --port $(APP_HOST_PORT)
...
vercel-preview-smoke:
	$(UV) run python -m scripts.vercel_preview_smoke --base-url "$(VERCEL_PREVIEW_URL)" --output $(ARTIFACTS_DIR)/vercel-preview-smoke.json
...
run:
	$(UV) run uvicorn ragrig.main:create_app --factory --host 0.0.0.0 --port 8000 --reload
...
google-workspace-diagnostics:
	$(UV) run python -m scripts.google_workspace_diagnostics --pretty --output $(ARTIFACTS_DIR)/google-workspace-diagnostics.json
...
database-source-check:
	$(UV) run python -m scripts.database_source_check --pretty --output $(ARTIFACTS_DIR)/database-source-check.json

cost-latency-check:
	$(UV) run python -m scripts.cost_latency_check --pretty --output $(ARTIFACTS_DIR)/cost-latency-check.json
...
eval-reindex-diff:
	$(UV) run python -m scripts.eval_reindex_diff --pretty --output $(ARTIFACTS_DIR)/eval-reindex-diff.json --markdown-output $(ARTIFACTS_DIR)/eval-reindex-diff.md

# ── Config comparison ─────────────────────────────────────────
# Compare multiple chunking/embedding configs: make eval-config-compare
# Override built-in small/medium/large with: make eval-config-compare EVAL_CONFIGS=path/to/configs.json
eval-config-compare:
	$(UV) run python -m scripts.eval_config_compare --pretty \
		$(if $(EVAL_CONFIGS),--configs $(EVAL_CONFIGS),) \
		--output $(ARTIFACTS_DIR)/eval-config-compare.json \
		--markdown-output $(ARTIFACTS_DIR)/eval-config-compare.md

graph-eval-compare:
	$(UV) run python -m scripts.graph_retrieval_eval_compare --pretty \
		--output $(ARTIFACTS_DIR)/graph-eval-compare.json \
		--markdown-output $(ARTIFACTS_DIR)/graph-eval-compare.md

demo-rc-gate:
	$(UV) run python -m scripts.demo_rc_gate --pretty \
		--output $(ARTIFACTS_DIR)/demo-rc-gate.json \
		--markdown-output $(ARTIFACTS_DIR)/demo-rc-gate.md
...
knowledge-map-check:
	$(UV) run python -m scripts.knowledge_map_check --pretty --output $(ARTIFACTS_DIR)/knowledge-map-check.json
...
nightly-evidence-smoke:
	$(UV) run python -m scripts.nightly_evidence_smoke --pretty

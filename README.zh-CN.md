...
该命令会启动临时 SQLite 应用，通过 Web Console 上传 Markdown/PDF/DOCX，验证 pipeline/chunk UI，并在 Playground 发起一次 grounded answer。它需要 `npm` 和本地 Chrome/Chromium；如果没有 Chrome，可设置 `RAGRIG_CONSOLE_E2E_BROWSER_CHANNEL=chromium`。

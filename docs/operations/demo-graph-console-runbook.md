...
starts the React Console at `http://127.0.0.1:8000/`.
...
| Opening | Knowledge Map shows entities, relations, claims, evidence, and feedback controls. |
| Core loop | Retrieval Lab shows `hybrid_graph`, compares `dense`, `graph`, and `hybrid_graph`, then shows graph context. |
...
1. Open Knowledge Map and confirm entities, relations, claims, evidence, and
   feedback counts are visible.
2. Open Retrieval Lab and confirm the `hybrid_graph` mode is visible.

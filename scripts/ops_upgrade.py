...
def run_migrations(dsn: str) -> dict[str, Any]:
    result: dict[str, Any] = {"name": "alembic_migration", "status": "pass", "detail": None}
    try:
        ...
        if proc.returncode != 0:
            result["status"] = "failure"
            result["detail"] = f"alembic upgrade failed: {proc.stderr or proc.stdout}"
        else:
            result["detail"] = "migrations applied successfully"
    except subprocess.TimeoutExpired:
        result["status"] = "failure"
        result["detail"] = "alembic upgrade timed out"
    except FileNotFoundError:
        result["status"] = "failure"
        result["detail"] = "alembic not found"
    except Exception as exc:
        result["status"] = "failure"
        result["detail"] = str(exc)
    return result
...
def run_upgrade(settings) -> dict[str, Any]:
    timestamp = datetime.now(timezone.utc)
    snapshot_id = timestamp.strftime("%Y%m%dT%H%M%SZ")
    head_revision = _get_head_revision()

    pre_revision = _get_current_revision(settings.database_url)
    ...
    checks.append(
        {
            "name": "pre_upgrade_revision",
            "status": "pass" if pre_revision else "failure",
            "detail": f"pre-upgrade revision: {pre_revision or 'unknown'}",
        }
    )
    checks.append(run_migrations(settings.database_url))

    post_revision = _get_current_revision(settings.database_url)
    checks.append(
        {
            "name": "post_upgrade_revision",
            "status": "pass" if post_revision == head_revision else "failure",
            "detail": f"post-upgrade revision: {post_revision or 'unknown'}, "
    ...
    post_counts = _count_entities(settings.database_url)
    count_mismatches: list[str] = []
    for key in pre_counts:
        if pre_counts[key] != post_counts.get(key, -1):
            count_mismatches.append(f"{key}: {pre_counts[key]} -> {post_counts.get(key)}")
    checks.append(
        {
            "name": "entity_count_stability",
            "status": "pass" if not count_mismatches else "failure",
            "detail": (
                "; ".join(count_mismatches) if count_mismatches else "entity counts unchanged"
            ),
        }
    )
    ...
    summary: dict[str, Any] = {
        ...
        "report_path": str(ARTIFACTS_DIR / "ops-upgrade-summary.json"),
    }

    summary = _redact(summary)
    _assert_no_raw_secrets(summary, "ops-upgrade-summary")
    return summary


def main() -> int:
    parser = argparse.ArgumentParser(description="RAGRig upgrade smoke")
    parser.add_argument("--output", default=str(ARTIFACTS_DIR / "ops-upgrade-summary.json"))
